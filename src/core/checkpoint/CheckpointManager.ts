...
export class CheckpointManager {
	...
	async presentMultifileDiff(
		...
		try {
			if (seeNewChangesSinceLastTaskCompletion) {
				...
				changedFiles = await this.checkpointTracker?.getDiffSet(lastTaskCompletedMessage?.lastCheckpointHash, hash)
	...
	async doesLatestTaskCompletionHaveNewChanges(clineMessages: ClineMessage[]): Promise<boolean> {
		...
		// Get last task completed
		const lastTaskCompletedMessage = findLast(clineMessages.slice(0, messageIndex), (m) => m.say === "completion_result")

		try {
			// Get changed files between current state and commit
			const changedFiles = await this.checkpointTracker?.getDiffSet(lastTaskCompletedMessage?.lastCheckpointHash, hash)
